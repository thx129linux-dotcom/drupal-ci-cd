...
class FormHooks {
  ...
  public static function preprocessInput(&$variables) {
    ...
    if ($element['#type'] === 'textfield' && !empty($attributes['class']) || $element['#type'] === 'password' && !empty($attributes['class'])) {
  ...
  public function formSearchBlockFormAlter(&$form, FormStateInterface $form_state, $form_id) {
    ...
    $form['keys']['#title'] = '';
    $form['keys']['#size'] = 20;
    $form['keys']['#placeholder'] = $this->t('Search');
  }
